    ...
    <div className="fixed inset-0 z-[90] overflow-y-auto bg-black/80 px-3 py-4 backdrop-blur-md sm:px-5 sm:py-8">
      ...
      <div className="relative mx-auto flex min-h-full w-full max-w-3xl items-center">
        <section
          ...
          tabIndex={-1}
          className="panel relative my-auto max-h-[calc(100vh-2rem)] w-full overflow-hidden rounded-[1.5rem] border-white/10 shadow-2xl shadow-black sm:max-h-[calc(100vh-4rem)] sm:rounded-[2rem]"
        >
          <div className="sticky top-0 z-10 border-b border-white/10 bg-neutral-950/95 px-4 py-4 backdrop-blur sm:px-6">
            <div className="flex items-start justify-between gap-4">
              <div className="min-w-0">
                <p className="intel-label mb-2 text-gray-100">Kill Log</p>
              ...
              <button
                type="button"
                onClick={onClose}
                className="flex h-11 w-11 shrink-0 items-center justify-center rounded-full border border-white/10 bg-white/5 text-gray-400 transition hover:border-white/10 hover:text-gray-100"
                aria-label="Close"
              >
          ...
          <div className="max-h-[calc(100vh-10rem)] overflow-y-auto p-4 sm:max-h-[calc(100vh-12rem)] sm:p-6">
            ...
            <div className="mt-5 overflow-hidden rounded-2xl border border-white/10 bg-black/20">
              <div className="flex items-center justify-between gap-3 border-b border-white/10 px-4 py-3">
                <div className="flex items-center gap-2 text-[0.68rem] font-black uppercase tracking-[0.18em] text-gray-400">
                  <Clock3 className="h-4 w-4 text-gray-100" aria-hidden="true" />
                  Latest first
                </div>
              ...
              {loading ? (
                <div className="flex items-center justify-center gap-3 px-4 py-10 text-sm font-black uppercase tracking-[0.18em] text-gray-400">
                  <Loader2 className="h-5 w-5 animate-spin text-gray-100" aria-hidden="true" />
                  Loading log
                </div>
              ) : error ? (
                    ...
                    return (
                      <li key={entry.killId} className="grid gap-3 px-4 py-4 sm:grid-cols-[2.5rem_minmax(0,1fr)_auto] sm:items-center">
                        ...
                        <div className="flex flex-wrap gap-2 sm:justify-end">
                          <span className="rounded-md border border-white/10 bg-white/5 px-2.5 py-1 text-[0.65rem] font-black uppercase tracking-[0.16em] text-gray-100">
            ...
            <div className="mt-4 overflow-hidden rounded-2xl border border-white/10 bg-black/20">
              <div className="flex items-center justify-between gap-3 border-b border-white/10 px-4 py-3">
                <div className="flex items-center gap-2 text-[0.68rem] font-black uppercase tracking-[0.18em] text-gray-400">
                  <Users className="h-4 w-4 text-gray-100" aria-hidden="true" />
                  Clan scoreboard
                </div>
                <span className="text-[0.68rem] font-black uppercase tracking-[0.18em] text-gray-500">
                ...
                <ol className="divide-y divide-white/10">
                  {clanKillTotals.map((clanTotal, index) => (
                    <li key={clanTotal.clanTag} className="grid grid-cols-[2.5rem_minmax(0,1fr)_auto] items-center gap-3 px-4 py-3">
                      ...
                      <span className="rounded-md border border-white/10 bg-white/5 px-3 py-1 text-[0.68rem] font-black uppercase tracking-[0.16em] text-gray-100">
