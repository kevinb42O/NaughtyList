...
function MessageReplyPreview({ currentUserId, message, onCancel, tone = 'bubble' }) {
  const bubbleTone = tone === 'composer'
    ? 'mb-2 border-white/10 bg-white/5 px-3 py-2 text-gray-50'
  ...
  return (
    <div className={`flex min-w-0 items-start gap-2 rounded-xl border ${bubbleTone}`}>
      <MessageSquareQuote className="mt-0.5 h-3.5 w-3.5 shrink-0 text-gray-200" aria-hidden="true" />
      <div className="min-w-0 flex-1">
        <p className="truncate text-[0.58rem] font-black uppercase tracking-[0.14em] text-gray-200/90">
