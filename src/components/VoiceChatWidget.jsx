        ...
        {isOpen ? (
          <section aria-label="Squad comms" className="w-[min(calc(100vw-2rem),22rem)] rounded-2xl border border-white/10 bg-zinc-950/95 p-4 shadow-[0_16px_50px_rgba(0,0,0,0.65)] backdrop-blur-2xl">
              ...
              {!isConnected ? (
                ...
                <div className="mt-3 space-y-3">
                      ...
                      return (
                        <div key={participant.id} className={`flex min-h-12 items-center justify-between rounded-xl border p-1.5 transition ${isSpeaking ? 'border-emerald-500/40 bg-emerald-500/10' : 'border-white/6 bg-zinc-900/45 hover:bg-zinc-800/75'}`}>
                          ...
                          <div className="flex shrink-0 items-center gap-1.5 px-1">
                            ...
                            {participant.muted ? <MicOff className="h-3.5 w-3.5 text-gray-400" aria-label="Muted" /> : <span className="h-1.5 w-1.5 rounded-full bg-emerald-500 opacity-70" aria-label="Mic on" />}
                          </div>
                        </div>
                      )
                    })}
                  ...
                  <div className="grid grid-cols-2 gap-2 border-t border-white/6 pt-3">
                    ...
                    <button type="button" aria-pressed={isDeafened} aria-label={isDeafened ? 'Turn sound on' : 'Deafen voice chat'} onClick={toggleDeafen} className={`flex min-h-14 flex-col items-center justify-center gap-1 rounded-xl border transition focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-cyan-300/80 ${isDeafened ? 'border-white/10 bg-white/5 text-gray-200' : 'border-white/8 bg-zinc-900/90 text-gray-300 hover:bg-zinc-800'}`}>
                    ...
                    <button type="button" onClick={disconnectVoice} className="col-span-2 flex min-h-12 items-center justify-center gap-2 rounded-xl border border-white/10 bg-white/5 text-[0.56rem] font-black uppercase tracking-[0.14em] text-gray-200 transition hover:bg-white/5 active:scale-95 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-white/20">
                      <PhoneOff className="h-4 w-4 text-gray-400" aria-hidden="true" />
                      Disconnect
                    </button>
                  </div>
                </div>
            ...
            <div className="mt-3 flex items-center justify-center gap-1.5 text-[0.52rem] font-bold uppercase tracking-[0.08em] text-gray-600">
              <Sparkles className="h-3 w-3 text-white/50" aria-hidden="true" />
