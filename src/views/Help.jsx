      ...
      <PhoneFrame title={`${item.platform} home screen`}>
        <div className="mt-4 rounded-2xl border border-white/10 bg-black/35 p-3">
          <div className="flex items-center justify-between gap-2 rounded-xl border border-white/10 bg-white/[0.04] px-3 py-2">
            ...
            <Icon className="h-4 w-4 shrink-0 text-gray-100" aria-hidden="true" />
          </div>
        ...
        <div className="mt-4 grid gap-2">
          {item.steps.map((step, index) => (
            <div key={step} className="flex items-center gap-2 rounded-2xl border border-white/10 bg-black/30 p-2.5">
              <span className="flex h-7 w-7 shrink-0 items-center justify-center rounded-xl border border-white/10 bg-white/5 text-[0.68rem] font-black text-gray-100">
                {index + 1}
              </span>
          ...
          return (
            <article key={step.label} className="rounded-[1.4rem] border border-white/10 bg-black/25 p-4">
              <div className="flex items-center justify-between gap-3">
                <span className="flex h-10 w-10 items-center justify-center rounded-2xl border border-white/10 bg-white/5 text-gray-100">
  ...
  return (
    <article className="rounded-[1.5rem] border border-white/10 bg-black/25 p-4">
      <div className="flex items-start gap-3">
        <span className="flex h-11 w-11 shrink-0 items-center justify-center rounded-2xl border border-white/10 bg-white/5 text-gray-100">
          <Icon className="h-5 w-5" aria-hidden="true" />
        </span>
        <div className="min-w-0">
          ...
          <Link
            to={item.to}
            className="mt-4 inline-flex min-h-10 items-center gap-2 rounded-full border border-white/10 bg-white/5 px-4 text-[0.68rem] font-black uppercase tracking-[0.18em] text-gray-300 transition hover:border-white/10 hover:text-gray-100"
          >
            {item.action}
  ...
  return (
    <section className="panel rounded-[1.8rem] p-5">
      <div className="mb-4 flex items-center gap-3">
        <span className="flex h-11 w-11 items-center justify-center rounded-2xl border border-white/10 bg-white/5 text-gray-100">
          <Icon className="h-5 w-5" aria-hidden="true" />
        </span>
        <div>
        ...
        {group.questions.map((item) => (
          <details key={item.question} className="group p-4 open:bg-white/[0.02]">
            <summary className="flex cursor-pointer list-none items-start justify-between gap-3 text-sm font-black uppercase tracking-[0.08em] text-gray-200 marker:hidden">
              ...
              <Plus className="mt-0.5 h-4 w-4 shrink-0 text-gray-200 transition group-open:rotate-45" aria-hidden="true" />
            </summary>
  ...
  return (
    <div>
      <PageHeader eyebrow="Help Desk" title="Field Manual">
        ...
        <Link
          to="/updates"
          className="mt-3 inline-flex min-h-9 items-center gap-2 rounded-full border border-white/10 bg-white/[0.04] px-3 text-[0.6rem] font-black uppercase tracking-[0.16em] text-gray-500 transition hover:border-white/10 hover:text-gray-100"
        >
        ...
        {guideSections.map((section) => {
          ...
          return (
            <a
              key={section.href}
              href={section.href}
              className="inline-flex min-h-11 items-center justify-center gap-2 rounded-2xl border border-white/10 bg-white/[0.04] px-3 text-[0.62rem] font-black uppercase tracking-[0.16em] text-gray-300 transition hover:border-white/10 hover:text-gray-100"
            >
      ...
      <section id="start" className="scroll-mt-24 mb-5 grid gap-4 lg:grid-cols-[minmax(0,0.95fr)_minmax(0,1.05fr)] lg:items-stretch">
          ...
          <div className="flex flex-col gap-5 sm:flex-row sm:items-center">
                ...
                {[
                  ...
                  return (
                    <div key={item.label} className="flex aspect-square flex-col items-center justify-center gap-1 rounded-2xl border border-white/10 bg-white/[0.04] text-center">
                      <Icon className="h-4 w-4 text-gray-100" aria-hidden="true" />
            ...
            <div className="min-w-0 flex-1">
              ...
              <div className="mt-5 flex flex-wrap gap-2">
                ...
                <span className="inline-flex items-center gap-2 rounded-full border border-white/10 bg-white/5 px-3 py-1.5 text-[0.62rem] font-black uppercase tracking-[0.18em] text-gray-100">
        ...
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-1">
          ...
          <Link to="/clans" className="rounded-[1.5rem] border border-white/10 bg-white/5 p-5 transition hover:bg-white/5">
            <UsersRound className="h-6 w-6 text-gray-100" aria-hidden="true" />
            <p className="mt-4 text-lg font-black uppercase tracking-[0.04em] text-white">Create or join a clan</p>
            <p className="mt-2 text-sm leading-6 text-gray-50/70">Clan HQ handles invites, requests, roles, and rooms.</p>
          </Link>
        </div>
      </section>
