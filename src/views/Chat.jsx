...
function isGroupedWithNeighbor(message, neighborMessage) {
  ...
  if (message.user_id !== neighborMessage.user_id) {
    return false
  }
        ...
        <div className="min-h-0 flex-1 overflow-y-auto pr-1">
          {showClanLoading ? (
            ...
            activeMessages.map((chatMessage, index) => {
              ...
              const startsGroup = !isGroupedWithNeighbor(chatMessage, previousMessage)
              const endsGroup = !isGroupedWithNeighbor(chatMessage, nextMessage)
              const showDateDivider = !previousMessage || !isSameDay(previousMessage.created_at, chatMessage.created_at)

              return (
                <div key={chatMessage.id} className="space-y-2 py-0.5">
                  {showDateDivider ? (
