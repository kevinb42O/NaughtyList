...
function ProfileInitial({ profile, mine, online }) {
  return <ProfileAvatar className={mine ? 'ring-white/20' : ''} profile={profile} online={online} showOnline size="sm" />
}
      ...
      <div className="flex gap-1.5 overflow-x-auto pb-0.5">
        {showMentionEveryoneSuggestion ? (
          <button
            ...
            className="flex min-w-[11rem] items-center gap-2 rounded-xl border border-white/10 bg-white/5 px-2.5 py-2 text-left transition hover:border-white/10 hover:bg-white/5"
          >
            <span className="inline-flex h-9 w-9 shrink-0 items-center justify-center rounded-full border border-white/10 bg-white/5 text-[0.62rem] font-black uppercase tracking-[0.12em] text-gray-100">
              All
            </span>
            <span className="min-w-0 flex-1">
              ...
              <span className="block truncate text-[0.62rem] font-bold text-gray-200/80">@all</span>
            </span>
          </button>
        ) : null}
        {mentionSuggestions.map((nextProfile) => (
          <button
            ...
            className="flex min-w-[11rem] items-center gap-2 rounded-xl border border-white/10 bg-white/[0.04] px-2.5 py-2 text-left transition hover:border-white/10 hover:bg-white/5"
          >
            ...
            <span className="min-w-0 flex-1">
              ...
              <span className="block truncate text-[0.62rem] font-bold text-gray-200/80">@{mentionHandle(nextProfile)}</span>
            </span>
          </button>
        ))}
      </div>
    ...
    <div>
      ...
      <section ref={chatPanelRef} style={chatPanelStyle} className={`chat-stable-panel flex h-[calc(var(--visual-viewport-height)-17rem)] min-h-0 flex-col rounded-[1.35rem] p-0 sm:h-[72vh] sm:min-h-[34rem] sm:rounded-[1.8rem] ${chatKeyboardActive ? 'chat-stable-panel--keyboard' : ''}`}>
        ...
        <div ref={scrollRef} onScroll={handleScroll} className="chat-scroll-surface min-h-0 flex-1 overflow-y-auto px-3 py-4 sm:px-4">
                  ...
                  <article className={`flex items-end gap-2 ${mine ? 'justify-end' : 'justify-start'}`}>
                    ...
                    <div className={`max-w-[86%] sm:max-w-[72%] ${mine ? 'items-end' : 'items-start'} flex flex-col`}>
                      <div className={`mb-1 flex max-w-full items-center gap-2 px-1 ${mine ? 'justify-end' : 'justify-start'}`}>
                        <span className={`truncate text-[0.64rem] font-black uppercase tracking-[0.1em] ${mine ? 'text-gray-200' : 'text-gray-300'}`}>
                      ...
                      {!wasDeleted && activeRoom === 'public' ? (
                        <button
                          ...
                          className="absolute -top-3 right-2 z-10 inline-flex h-7 w-7 items-center justify-center rounded-full border border-white/10 bg-zinc-950 text-gray-400 shadow-lg shadow-black/40 transition hover:border-white/10 hover:bg-white/5 hover:text-gray-100"
          ...
          {typingLabel ? (
            <div className="px-2 pb-1 pt-0 text-xs font-bold text-gray-200/75">{typingLabel}</div>
          ) : null}
        </div>
