export function mapPlayerFromSupabase(row, profileById = new Map()) {
  ...
  return {
    ...
    lastKillUserId: row.last_kill_user_id ?? null,
    lastKillDisplayName: row.last_kill_display_name ?? '',
    lastKillProfileClanTag: row.last_kill_profile_clan_tag ?? '',
    lastKillUserTotal: row.last_kill_user_total ?? 0,
    lastKillClanId: row.last_kill_clan_id ?? null,
    lastKillClanTag: row.last_kill_clan_tag ?? '',
    lastKillClanTotal: row.last_kill_clan_total ?? null,
    lastKillAt: row.last_kill_at ?? null,
    tags: row.tags ?? [],
    ...
    moderationStatus: row.moderation_status ?? 'unreviewed',
